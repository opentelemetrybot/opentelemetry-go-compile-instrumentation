...
- 指定项目打算遵守的语义约定版本
- 必须与 `instrumentation/**/semconv/` Go 代码中使用的 `semconv` 导入匹配
- 由 CI 验证以确保一致性
...
**何时使用**：在提交对 `instrumentation/**/semconv/` 中语义约定定义的更改之前运行此命令。
...
- 决定是否升级到更新版本
- 修改 `instrumentation/**/semconv/` 之前查看变更
...
1. 将属性定义添加到 `instrumentation/**/semconv/` 中的适当文件
...
当你修改 `instrumentation/**/semconv/` 或 `.semconv-version` 中的文件时：
