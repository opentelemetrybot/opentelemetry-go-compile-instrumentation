...
That's enough to put it in the build graph — you don't need to actually construct or use a `TracerProvider` yourself. Without it, spans are still created and exported correctly, but `log` output will not carry `trace_id`/`span_id`.
